...
```{r}
mpsz_HDB <- st_transform(mpsz_HDB, 3414)
st_crs(mpsz_HDB)
clinics_combined <- st_crs(clinics_combined, 3414)
st_crs(clinics_combined)
```

```{r}
#assign capacity of 2 per clinic
clinics_combined['capacity'] <- 2
st_crs(clinics_combined) <- st_crs(mpsz)
clinics_combined <- st_join(clinics_combined, mpsz)
```

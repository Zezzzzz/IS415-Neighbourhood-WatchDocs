...
```{r}
mpsz_HDB_filtered <- mpsz_HDB[mpsz_HDB$SUBZONE_N=="NORTHLAND", ]
clinics_combined_filtered <- clinics_combined[clinics_combined$SUBZONE_N=="NORTHLAND", ]
...
subzone_clinics <- st_as_sf(clinics_combined_filtered_sp, coords = c("LAT", "LONG"), crs = st_crs(mpsz)) %>% mutate(capacity = capacity)
...
for(i in 1:nrow(subzone_clinics)) {
  ...
  if(nrow(clinic_n_allocation) >= capacity) {
    ...
    # get the allocated elderlys, append to total allocated elderlys
    allocated_elderly <- clinic_n_allocation %>% slice(1:capacity)
    total_allocated_elderly <- rbind(total_allocated_elderly, allocated_elderly)
...
```{r}
df1 <- total_unallocated_elderly %>%
    select("blk_no_street","LAT","LONG","SUBZONE_N") %>% st_set_geometry(NULL)

df2 <- mpsz_HDB_split_sf %>% st_set_geometry(NULL)

total_allocated_elderly <- anti_join(df2, df1)
total_allocated_elderly$LONG1 <- total_allocated_elderly$LONG
total_allocated_elderly$LAT1 <- total_allocated_elderly$LAT

total_allocated_elderly <- st_as_sf(total_allocated_elderly, coords = c("LONG1","LAT1"),
                                         crs = 4326)
